...
class TestCompressionPassiveDetection:
    def test_passive_detection_triggers_on_shrink(self):
        ...
        # Mock _call_provider to avoid litellm/mock issues
    ...
    def test_passive_detection_triggers_on_shrink_real_call(self, mock_litellm):
    ...
    @patch("litellm.completion")
    def test_passive_detection_does_not_trigger_on_normal_growth(self, mock_litellm):
        mock_litellm.return_value = MagicMock()
        wrapper = mock_wrapper_with_identity()
        wrapper._client = None
        wrapper._compression_guard = MagicMock(wraps=wrapper._compression_guard)
    ...
    @patch("litellm.completion")
    def test_passive_detection_respects_config_toggle(self, mock_litellm):
        mock_litellm.return_value = MagicMock()
        wrapper = mock_wrapper_with_identity()
        wrapper._client = None
    ...
    @patch("litellm.completion")
    def test_end_to_end_drift_recording(self, mock_litellm):
        mock_litellm.return_value = MagicMock()
        wrapper = mock_wrapper_with_identity()
        wrapper._client = None
