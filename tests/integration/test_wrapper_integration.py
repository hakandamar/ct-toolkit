...
@pytest.fixture
def wrapper(mock_litellm, tmp_path):
    ...
    # Patch litellm.completion here to ensure it's captured when wrapper.chat() calls it
...
class TestWrapperIntegration:
    ...
    def test_basic_chat_flow(self, mock_divergence, wrapper, mock_litellm):
        """Test a normal chat completion flow."""
        mock_divergence.return_value = 0.0
        response = wrapper.chat("Hello, what can you do?")
        
        # Verify the wrapper called litellm correctly
    ...
    @patch("ct_toolkit.identity.embedding.IdentityEmbeddingLayer.compute_divergence")
    def test_l1_warning_tier_triggered(self, mock_divergence, wrapper, mock_litellm):
    ...
    @patch("ct_toolkit.identity.embedding.IdentityEmbeddingLayer.compute_divergence")
    def test_l2_judge_triggered_and_aligned(self, mock_divergence, mock_icm_run, wrapper, mock_litellm):
    ...
    @patch("ct_toolkit.divergence.l3_icm.ICMRunner.run")
    @patch("ct_toolkit.identity.embedding.IdentityEmbeddingLayer.compute_divergence")
    def test_l3_icm_escalation_from_l2_misaligned(self, mock_divergence, mock_icm_run, wrapper, mock_litellm):
    ...
    @patch("ct_toolkit.divergence.l3_icm.ICMRunner.run")
    @patch("ct_toolkit.identity.embedding.IdentityEmbeddingLayer.compute_divergence")
    def test_l3_icm_direct_trigger(self, mock_divergence, mock_icm_run, wrapper, mock_litellm):
    ...
    def test_logs_are_persisted(self, wrapper, mock_litellm):
